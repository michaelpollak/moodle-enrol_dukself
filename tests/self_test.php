...
/**
 * Dukself enrolment plugin tests.
 *
...
global $CFG;
require_once($CFG->dirroot.'/enrol/dukself/lib.php');
require_once($CFG->dirroot.'/enrol/dukself/locallib.php');

class enrol_dukself_testcase extends advanced_testcase {

    public function test_basics() {
        $this->assertTrue(enrol_is_enabled('dukself'));
        $plugin = enrol_get_plugin('dukself');
        $this->assertInstanceOf('enrol_dukself_plugin', $plugin);
        $this->assertEquals(1, get_config('enrol_dukself', 'defaultenrol'));
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, get_config('enrol_dukself', 'expiredaction'));
    }

    public function test_sync_nothing() {
        global $SITE;

        $selfplugin = enrol_get_plugin('dukself');
    ...
    public function test_longtimnosee() {
        ...
        $this->assertEquals(3, $DB->count_records('enrol', array('enrol'=>'dukself')));
        $instance1 = $DB->get_record('enrol', array('courseid'=>$course1->id, 'enrol'=>'dukself'), '*', MUST_EXIST);
        $instance2 = $DB->get_record('enrol', array('courseid'=>$course2->id, 'enrol'=>'dukself'), '*', MUST_EXIST);
        $instance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'dukself'), '*', MUST_EXIST);
    ...
    public function test_send_expiry_notifications() {
        ...
        /** @var $selfplugin enrol_dukself_plugin */
        $selfplugin = enrol_get_plugin('dukself');
        ...
        $this->assertEquals(4, $DB->count_records('enrol', array('enrol'=>'dukself')));
        ...
        $instance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'dukself'), '*', MUST_EXIST);
    ...
    public function test_show_enrolme_link() {
        ...
        // Cohort member test.
        $instance5 = $DB->get_record('enrol', array('courseid'=>$course5->id, 'enrol'=>'dukself'), '*', MUST_EXIST);
        ...
        $instance7 = $DB->get_record('enrol', array('courseid'=>$course6->id, 'enrol'=>'dukself'), '*', MUST_EXIST);
        ...
        $instance8 = $DB->get_record('enrol', array('courseid'=>$course7->id, 'enrol'=>'dukself'), '*', MUST_EXIST);
        ...
        $instance9 = $DB->get_record('enrol', array('courseid'=>$course8->id, 'enrol'=>'dukself'), '*', MUST_EXIST);
        ...
        $instance10 = $DB->get_record('enrol', array('courseid'=>$course9->id, 'enrol'=>'dukself'), '*', MUST_EXIST);
        ...
        $instance11 = $DB->get_record('enrol', array('courseid'=>$course10->id, 'enrol'=>'dukself'), '*', MUST_EXIST);
        ...
        $instance12 = $DB->get_record('enrol', array('courseid'=>$course11->id, 'enrol'=>'dukself'), '*', MUST_EXIST);
     ...
     */
    public function test_can_dukself_enrol() {
        ...
        $expectederrorstring = get_string('canntenrol', 'enrol_dukself');
        ...
        $this->assertSame($noaccesshtml, $selfplugin->can_dukself_enrol($instance1, true));

        $this->setUser($user1);
        $this->assertTrue($selfplugin->can_dukself_enrol($instance1, true));

        // Active enroled user.
        $this->setUser($user2);
        $selfplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $this->setUser($user1);
        $this->assertSame($expectederrorstring, $selfplugin->can_dukself_enrol($instance1, true));
    }

    /**
     * Test enrol_dukself_check_group_enrolment_key
     */
    public function test_enrol_dukself_check_group_enrolment_key() {
        global $DB;
        ...
        $result = enrol_dukself_check_group_enrolment_key($course->id, 'invalidpassword');
        $this->assertFalse($result);

        $result = enrol_dukself_check_group_enrolment_key($course->id, 'thepassword');
        ...
        $result = enrol_dukself_check_group_enrolment_key($othercourse->id, 'thepassword');
        $this->assertFalse($result);
    ...
    public function test_get_welcome_email_contact() {
        ...
        $selfplugin = enrol_get_plugin('dukself');
        $instance1 = $DB->get_record('enrol', ['courseid' => $course1->id, 'enrol' => 'dukself'], '*', MUST_EXIST);
        $instance1->customint6 = 1;
        ...
        assign_capability('enrol/dukself:holdkey', CAP_ALLOW, $managerrole->id, $context);
    ...
    public function test_get_user_enrolment_actions() {
        ...
        // Dukself enrol has 2 enrol actions -- edit and unenrol.
