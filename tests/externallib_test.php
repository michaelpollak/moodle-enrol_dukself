...
/**
 * Dukself enrol external PHPunit tests
 *
...
require_once($CFG->dirroot . '/enrol/dukself/externallib.php');

class enrol_dukself_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_get_instance_info() {
        ...
        // Check if dukself enrolment plugin is enabled.
        ...
        $this->setAdminUser();
        $instanceinfo1 = enrol_dukself_external::get_instance_info($instanceid1);
        $instanceinfo1 = external_api::clean_returnvalue(enrol_dukself_external::get_instance_info_returns(), $instanceinfo1);

        $this->assertEquals($instanceid1, $instanceinfo1['id']);
        $this->assertEquals($course->id, $instanceinfo1['courseid']);
        $this->assertEquals('dukself', $instanceinfo1['type']);
        ...
        $instanceinfo2 = enrol_dukself_external::get_instance_info($instanceid2);
        $instanceinfo2 = external_api::clean_returnvalue(enrol_dukself_external::get_instance_info_returns(), $instanceinfo2);
        $this->assertEquals($instanceid2, $instanceinfo2['id']);
        $this->assertEquals($course->id, $instanceinfo2['courseid']);
        $this->assertEquals('dukself', $instanceinfo2['type']);
        $this->assertEquals('Test instance 2', $instanceinfo2['name']);
        $this->assertEquals(get_string('canntenrol', 'enrol_dukself'), $instanceinfo2['status']);
        $this->assertFalse(isset($instanceinfo2['enrolpassword']));

        $instanceinfo3 = enrol_dukself_external::get_instance_info($instanceid3);
        $instanceinfo3 = external_api::clean_returnvalue(enrol_dukself_external::get_instance_info_returns(), $instanceinfo3);
        $this->assertEquals($instanceid3, $instanceinfo3['id']);
        $this->assertEquals($course->id, $instanceinfo3['courseid']);
        $this->assertEquals('dukself', $instanceinfo3['type']);
        $this->assertEquals('Test instance 3', $instanceinfo3['name']);
        $this->assertTrue($instanceinfo3['status']);
        $this->assertEquals(get_string('password', 'enrol_dukself'), $instanceinfo3['enrolpassword']);

        // Try to retrieve information using a normal user for a hidden course.
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
        try {
            enrol_dukself_external::get_instance_info($instanceid3);
    ...
    public function test_enrol_user() {
        ...
        // Dukself enrol me.
        $result = enrol_dukself_external::enrol_user($course1->id);
        $result = external_api::clean_returnvalue(enrol_dukself_external::enrol_user_returns(), $result);
        ...
        try {
            enrol_dukself_external::enrol_user($course2->id);
        } catch (moodle_exception $e) {
        ...
        $result = enrol_dukself_external::enrol_user($course2->id);
        $result = external_api::clean_returnvalue(enrol_dukself_external::enrol_user_returns(), $result);
        ...
        $result = enrol_dukself_external::enrol_user($course2->id, 'invalidkey');
        $result = external_api::clean_returnvalue(enrol_dukself_external::enrol_user_returns(), $result);
        self::assertFalse($result['status']);
        self::assertCount(1, $result['warnings']);
        self::assertEquals('4', $result['warnings'][0]['warningcode']);
        ...
        $result = enrol_dukself_external::enrol_user($course2->id, 'abcdef');
        $result = external_api::clean_returnvalue(enrol_dukself_external::enrol_user_returns(), $result);
        ...
        // Now, everything ok.
        $result = enrol_dukself_external::enrol_user($course2->id, 'zyx');
        $result = external_api::clean_returnvalue(enrol_dukself_external::enrol_user_returns(), $result);
        ...
        self::setUser($user4);
        $result = enrol_dukself_external::enrol_user($course2->id, 'abcdef', $instance3id);
        $result = external_api::clean_returnvalue(enrol_dukself_external::enrol_user_returns(), $result);
        self::assertTrue($result['status']);
        self::assertTrue(is_enrolled($context2, $user3));
