...
/**
 * Dukself enrolment plugin based on enrol_self.
 *
...
/**
 * Dukself enrolment plugin implementation.
 * @author Petr Skoda with extensions from Michael Pollak
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_dukself_plugin extends enrol_plugin {
    ...
    public function get_info_icons(array $instances) {
        ...
        foreach ($instances as $instance) {
            if ($this->can_dukself_enrol($instance, false) !== true) {
        ...
        $icons = array();
        if ($nokey) {
            $icons[] = new pix_icon('withoutkey', get_string('pluginname', 'enrol_dukself'), 'enrol_dukself');
        }
        if ($key) {
            $icons[] = new pix_icon('withkey', get_string('pluginname', 'enrol_dukself'), 'enrol_dukself');
        }
        return $icons;
    }
    ...
    public function show_enrolme_link(stdClass $instance) {
        ...
        if (true !== $this->can_dukself_enrol($instance, false)) {
            return false;
        }
    ...
    public function can_add_instance($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/dukself:config', $context)) {
            return false;
        }
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        require_once("$CFG->dirroot/enrol/dukself/locallib.php");

        $enrolstatus = $this->can_dukself_enrol($instance);

        if (true === $enrolstatus) {
            // This user can self enrol using this instance.
            $form = new enrol_dukself_enrol_form(null, $instance);
            $instanceid = optional_param('instance', 0, PARAM_INT);
            if ($instance->id == $instanceid) {
                if ($data = $form->get_data()) {
                    $this->enrol_dukself($instance, $data);
                }
            }
        } else {
            ...
            // The can_dukself_enrol call returns a button to the login page if the user is a
            // guest, setting the login url to the form if that is the case.
            $url = isguestuser() ? get_login_url() : null;
            $form = new enrol_dukself_empty_form($url, $data);
        }
     ...
     */
    public function can_dukself_enrol(stdClass $instance, $checkuserenrolment = true) {
        ...
        if ($checkuserenrolment) {
            ...
            if ($DB->get_record('user_enrolments', array('userid' => $USER->id, 'enrolid' => $instance->id))) {
                return get_string('canntenrol', 'enrol_dukself');
            }
        }
        ...
        if ($instance->enrolstartdate != 0 and $instance->enrolstartdate > time()) {
            return get_string('canntenrolearly', 'enrol_dukself', userdate($instance->enrolstartdate));
        }

        if ($instance->enrolenddate != 0 and $instance->enrolenddate < time()) {
            return get_string('canntenrollate', 'enrol_dukself', userdate($instance->enrolenddate));
        }
        ...
        if ($instance->customint3 > 0) {
            ...
            if ($count >= $instance->customint3) {
                ...
                return get_string('maxenrolledreached', 'enrol_dukself');
            }
        }
        ...
        if ($instance->customint5) {
            ...
            if (!cohort_is_member($instance->customint5, $USER->id)) {
                ...
                return markdown_to_html(get_string('cohortnonmemberinfo', 'enrol_dukself', $a));
            }
        }
    ...
    public function get_enrol_info(stdClass $instance) {
        ...
        $instanceinfo->status = $this->can_dukself_enrol($instance);

        if ($instance->password) {
            $instanceinfo->requiredparam = new stdClass();
            $instanceinfo->requiredparam->enrolpassword = get_string('password', 'enrol_dukself');
        }
        ...
        if ((true === $instanceinfo->status) && $instance->password) {
            $instanceinfo->wsfunction = 'enrol_dukself_get_instance_info';
        }
        return $instanceinfo;
    }
    ...
    public function get_instance_defaults() {
        ...
        $fields['customint7']      = 0; // Autosuspend has no admin setting.
        // </mod by DUK>
    ...
    protected function email_welcome_message($instance, $user) {
        ...
        if (trim($instance->customtext1) !== '') {
            ...
            $messagetext = get_string('welcometocoursetext', 'enrol_dukself', $a);
            $messagehtml = text_to_html($messagetext, null, false, true);
        }

        $subject = get_string('welcometocourse', 'enrol_dukself', format_string($course->fullname, true, array('context'=>$context)));
    ...
    public function sync(progress_trace $trace, $courseid = null) {
        ...
        $trace->output('Verifying dukself-enrolments...');
        ...
        // <mod by DUK @michaelpollak>
        // Autosuspend, check for users that have completed the course if autosuspend (customint7) is true.
        $instances = array();
        $sql = "SELECT ue.*, e.courseid, c.id AS contextid
                FROM {user_enrolments} ue
                JOIN {enrol} e ON (e.id = ue.enrolid AND e.enrol = 'dukself' AND e.customint7 = 1)
                JOIN {context} c ON (c.instanceid = e.courseid AND c.contextlevel = :courselevel)
                JOIN {course_completions} cc ON (cc.course = e.courseid AND cc.timecompleted <> NULL)
                WHERE ue.status = :useractive
                $coursesql";
        ...
        foreach ($rs as $ue) {
            ...
            // We suspend user, change status to 1.
            $this->update_user_enrol($instance, $ue->userid, ENROL_USER_SUSPENDED);
            $trace->output("Suspending user $ue->userid from course $instance->courseid because course was completed already.", 1);
        }
        ...
        // Note: the logic of dukself enrolment guarantees that user logged in at least once (=== u.lastaccess set)
        ...
        $sql = "SELECT e.*, ue.userid
                  FROM {user_enrolments} ue
                  JOIN {enrol} e ON (e.id = ue.enrolid AND e.enrol = 'dukself' AND e.customint2 > 0)
        ...
        $trace->output('...user dukself-enrolment updates finished.');
        $trace->finished();
    ...
    /**
     * Returns the user who is responsible for dukself enrolments in given instance.
     *
     * Usually it is the first editing teacher - the person with "highest authority"
     * as defined by sort_by_roleassignment_authority() having 'enrol/dukself:manage'
     * capability.
     *
    ...
    protected function get_enroller($instanceid) {
        ...
        if ($users = get_enrolled_users($context, 'enrol/dukself:manage')) {
        ...
        if ($merge and $instances = $DB->get_records('enrol', $merge, 'id')) {
            ...
            if (!empty($data->customint5)) {
                if ($step->get_task()->is_samesite()) {
                    ...
                    // Use some id that can not exist in order to prevent dukself enrolment,
    ...
    public function restore_role_assignment($instance, $roleid, $userid, $contextid) {
        ...
        // we do not use component in manual or dukself enrol.
    ...
    public function can_delete_instance($instance) {
        $context = context_course::instance($instance->courseid);
        return has_capability('enrol/dukself:config', $context);
    }
    ...
    public function can_hide_show_instance($instance) {
        ...
        if (!has_capability('enrol/dukself:config', $context)) {
            return false;
        }
    ...
    /**
     * The dukself enrollment plugin has several bulk operations that can be performed.
    ...
    public function get_bulk_operations(course_enrolment_manager $manager) {
        ...
        if (has_capability("enrol/dukself:manage", $context)) {
            $bulkoperations['editselectedusers'] = new enrol_dukself_editselectedusers_operation($manager, $this);
        }
        if (has_capability("enrol/dukself:unenrol", $context)) {
            $bulkoperations['deleteselectedusers'] = new enrol_dukself_deleteselectedusers_operation($manager, $this);
        }
    ...
    public function edit_instance_form($instance, MoodleQuickForm $mform, $context) {
        ...
        $mform->addElement('select', 'status', get_string('status', 'enrol_dukself'), $options);
        $mform->addHelpButton('status', 'status', 'enrol_dukself');

        $options = $this->get_newenrols_options();
        $mform->addElement('select', 'customint6', get_string('newenrols', 'enrol_dukself'), $options);
        $mform->addHelpButton('customint6', 'newenrols', 'enrol_dukself');
        $mform->disabledIf('customint6', 'status', 'eq', ENROL_INSTANCE_DISABLED);

        $passattribs = array('size' => '20', 'maxlength' => '50');
        $mform->addElement('passwordunmask', 'password', get_string('password', 'enrol_dukself'), $passattribs);
        $mform->addHelpButton('password', 'password', 'enrol_dukself');
        ...
        $mform->addElement('select', 'customint1', get_string('groupkey', 'enrol_dukself'), $options);
        $mform->addHelpButton('customint1', 'groupkey', 'enrol_dukself');

        $roles = $this->extend_assignable_roles($context, $instance->roleid);
        $mform->addElement('select', 'roleid', get_string('role', 'enrol_dukself'), $roles);

        $options = array('optional' => true, 'defaultunit' => 86400);
        $mform->addElement('duration', 'enrolperiod', get_string('enrolperiod', 'enrol_dukself'), $options);
        $mform->addHelpButton('enrolperiod', 'enrolperiod', 'enrol_dukself');

        // <mod by DUK @michaelpollak>
        // Extension autosuspend. User will be suspended after completion.
        $options = array(0 => get_string('no'), 1 => get_string('yes'));
        $mform->addElement('select', 'customint7', get_string('autosuspend', 'enrol_dukself'), $options);
        $mform->addHelpButton('customint7', 'autosuspend', 'enrol_dukself');
        // </mod by DUK>
        ...
        $mform->addElement('date_time_selector', 'enrolstartdate', get_string('enrolstartdate', 'enrol_dukself'), $options);
        $mform->setDefault('enrolstartdate', 0);
        $mform->addHelpButton('enrolstartdate', 'enrolstartdate', 'enrol_dukself');

        $options = array('optional' => true);
        $mform->addElement('date_time_selector', 'enrolenddate', get_string('enrolenddate', 'enrol_dukself'), $options);
        $mform->setDefault('enrolenddate', 0);
        $mform->addHelpButton('enrolenddate', 'enrolenddate', 'enrol_dukself');

        $options = $this->get_longtimenosee_options();
        $mform->addElement('select', 'customint2', get_string('longtimenosee', 'enrol_dukself'), $options);
        $mform->addHelpButton('customint2', 'longtimenosee', 'enrol_dukself');

        $mform->addElement('text', 'customint3', get_string('maxenrolled', 'enrol_dukself'));
        $mform->addHelpButton('customint3', 'maxenrolled', 'enrol_dukself');
        ...
        if (count($cohorts) > 1) {
            $mform->addElement('select', 'customint5', get_string('cohortonly', 'enrol_dukself'), $cohorts);
            $mform->addHelpButton('customint5', 'cohortonly', 'enrol_dukself');
        } else {
        ...
        $mform->addElement('select', 'customint4', get_string('sendcoursewelcomemessage', 'enrol_dukself'),
                enrol_send_welcome_email_options());
        $mform->addHelpButton('customint4', 'sendcoursewelcomemessage', 'enrol_dukself');

        $options = array('cols' => '60', 'rows' => '8');
        $mform->addElement('textarea', 'customtext1', get_string('customwelcomemessage', 'enrol_dukself'), $options);
        $mform->addHelpButton('customtext1', 'customwelcomemessage', 'enrol_dukself');
    ...
    public function edit_instance_validation($data, $files, $instance, $context) {
        ...
        if ($data['status'] == ENROL_INSTANCE_ENABLED) {
            if (!empty($data['enrolenddate']) and $data['enrolenddate'] < $data['enrolstartdate']) {
                $errors['enrolenddate'] = get_string('enrolenddaterror', 'enrol_dukself');
            }
        }
    ...
    /**
     * Gets a list of roles that this user can assign for the course as the default for dukself-enrolment.
     *
     * @param context $context the context.
     * @param integer $defaultrole the id of the role that is set as the default for dukself-enrolment
    ...
    public function get_welcome_email_contact($sendoption, $context) {
        ...
        if ($sendoption == ENROL_SEND_EMAIL_FROM_COURSE_CONTACT) {
            ...
            // Send as the first user with enrol/dukself:holdkey capability assigned in the course.
            list($sort) = users_order_by_sql('u');
            $keyholders = get_users_by_capability($context, 'enrol/dukself:holdkey', 'u.*', $sort);
            if (!empty($keyholders)) {
 ...
 */
function enrol_dukself_get_fontawesome_icon_map() {
    return [
        'enrol_dukself:withkey' => 'fa-key',
        'enrol_dukself:withoutkey' => 'fa-sign-in',
    ];
}
