...
/**
 * Dukself enrol plugin external functions
 *
...
/**
 * Dukself enrolment external functions.
 *
 * @package   enrol_dukself
 ...
 * @since     Moodle 2.6
 */
class enrol_dukself_external extends external_api {
    ...
    public static function get_instance_info_parameters() {
        return new external_function_parameters(
                array('instanceid' => new external_value(PARAM_INT, 'instance id of dukself enrolment plugin.'))
            );
    }

    /**
     * Return dukself-enrolment instance information.
     *
     * @param int $instanceid instance id of dukself enrolment plugin.
    ...
    public static function get_instance_info($instanceid) {
        ...
        // Retrieve dukself enrolment plugin.
        $enrolplugin = enrol_get_plugin('dukself');
    ...
    public static function enrol_user_parameters() {
        return new external_function_parameters(
            array(
                ...
                'instanceid' => new external_value(PARAM_INT, 'Instance id of dukself enrolment plugin.', VALUE_DEFAULT, 0)
            )
        );
    }

    /**
     * Dukself enrol the current user in the given course.
     *
    ...
    public static function enrol_user($courseid, $password = '', $instanceid = 0) {
        ...
        // Retrieve the dukself enrolment plugin.
        $enrol = enrol_get_plugin('dukself');
        if (empty($enrol)) {
            throw new moodle_exception('canntenrol', 'enrol_dukself');
        }

        // We can expect multiple dukself-enrolment instances.
        $instances = array();
        $enrolinstances = enrol_get_instances($course->id, true);
        foreach ($enrolinstances as $courseenrolinstance) {
            if ($courseenrolinstance->enrol == "dukself") {
                // Instance specified.
        ...
        foreach ($instances as $instance) {
            $enrolstatus = $enrol->can_dukself_enrol($instance);
            if ($enrolstatus === true) {
                if ($instance->password and $params['password'] !== $instance->password) {
                    ...
                    if ($instance->customint1) {
                        ...
                        if (!enrol_dukself_check_group_enrolment_key($course->id, $params['password'])) {
                            $warnings[] = array(
                                'item' => 'instance',
                                'itemid' => $instance->id,
                                'warningcode' => '2',
                                'message' => get_string('passwordinvalid', 'enrol_dukself')
                            );
                            continue;
                        }
                    } else {
                        if ($enrol->get_config('showhint')) {
                            ...
                            $warnings[] = array(
                                ...
                                'itemid' => $instance->id,
                                'warningcode' => '3',
                                'message' => s(get_string('passwordinvalidhint', 'enrol_dukself', $hint)) // message is PARAM_TEXT.
                            );
                            continue;
                        } else {
                            $warnings[] = array(
                ...
                $enrol->enrol_dukself($instance, (object) $data);
                $enrolled = true;
                break;
            } else {
                $warnings[] = array(
